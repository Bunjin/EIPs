...
We should later develop wallet tools that would allow an user to properly fund her distinct personas in a non correlatable way, and even allow for transactions between personas. These tools could for instance include privacy preserving ways of transferring cryptocurrency (eg. Mixers, Znarks...), analytics to verify at any given time that personas are not correlatable (this should be based on a proper measure of likelihood of correlation).
